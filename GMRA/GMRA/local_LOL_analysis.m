function gMRA = local_LOL_analysis(gMRA,node)
...
LabelsInNode 	    = gMRA.Y(PointsInThisNode);
...
%     disp('LOL_really')
    ...
    types{1} = 'DENL';
    Kmax = gMRA.opts.ManifoldDimension;
    [Proj, ~] = LOL(lPtsInNode,LabelsInNode,types,Kmax);
    %gMRA.ScalFuns{node} = Proj{1}.V(:,1:gMRA.opts.ManifoldDimension);
    gMRA.ScalFuns{node} = Proj{1}.V';
    %disp('size of LOL Projection matrix')
    %size(Proj{1}.V)
end
