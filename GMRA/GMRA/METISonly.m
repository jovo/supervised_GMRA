function gMRA = METISonly(X,opts,Y, X_test)
...
%% LOL for the nodes in each scale
Scale = 3;
nodeInScale = find(gMRA.Scales == Scale);

% Find the nearest leaf to each data point
[~, gMRA.nearestCenters, gmra.dists2NearestLeafNodes] = nrsearch(cat(2,gMRA.Centers{nodeInScale}), X_test, 1, 0, [], struct('ReturnAsArrays',1));
gMRA.NodeLabels = nodeInScale(gMRA.nearestCenters);
% gMRA.leafNodeSizes = zeros(1,nLeafNodes);
% 

for i = 1: numel(nodeInScale)
    %Train data
    nodeIdx = nodeInScale(i);
    IdxNode = gMRA.PointsInNet{nodeIdx};
    X_train_node = X(:, IdxNode);
    Y_train_node = Y(:, IdxNode);
   
    % Test data
    netPts{i} = find(gMRA.NodeLabels == nodeIdx);
    X_test_node = X_test(:, netPts{i});
    
    % LOL in each nodes
    Opts.LOL_alg = 'DENL';
    [Opts.task, Opts.ks] = set_task_LOL( Opts, size(X_train_node,1) );
    %     Opts.task.ks = 20;
    kslist = Opts.ks
   
    for k = 1: numel(kslist)
        Opts.task.ks = kslist(k);
        
        tic;
        Timing_onset_LOL = cputime;
        [labels_pred_LOL, n_errors_LOL, classifier_LOL, ~] = LOL_traintest(X_train_node, Y_train_node, X_test_node, [], Opts );
        gMRA.Time.tictoc_LOL(k) = toc;
        gMRA.Time.cputime_LOL(k) = cputime - Timing_onset_LOL;
        
        gMRA.labels_pred_METISLOL(k, netPts{i}) = labels_pred_LOL;
    end
end
