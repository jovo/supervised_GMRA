...
if testLDA ~= 2
    ...
    %% Run differnt ks Separately
    SeparateK = input('TestAllK at once: 0/ separately: 1 \n');
...
%	Opts.task.ks =[1 400];
        ...
        if SeparateK
            kslist = Opts.task.ks;
            for oneK = 1: numel(Opts.task.ks)
                Opts.task.ks = kslist(oneK);
                tic;
                Timing_onset_LOL = cputime;
                [labels_pred_LOL, n_errors_LOL, ~, ~] = LOL_traintest( data_train', labels_train, data_test', labels_test, Opts );
                tictoc_LOL(oneK, nT) = toc;
                cputime_LOL(oneK, nT) = cputime - Timing_onset_LOL;
                ...
                ACC_LOL(oneK,nT) = 1- ERR_LOL./numel(labels_test);
