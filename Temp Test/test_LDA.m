...
Target = [ 1 1 1 2 2 2]';
[classifier_trained] = LDA_train(Input,Target);
classifier_trained.W
classifier_trained.ClassLabel

figure; gscatter(Input(:,1), Input(:,2), Target)
hold on
K3 = classifier_trained.W(1,1)-classifier_trained.W(2,1);
L3 = classifier_trained.W(1,2:end) - classifier_trained.W(2,2:end);
% Plot the curve K + [x,y]*L + [x,y]*Q*[x,y]' = 0:
f3 = @(x,y) K3 + L3(1)*x + L3(2)*y
% f3 = @(x,y) 97.5 - 6*x - 5*y
...
figure; gscatter(Input(:,1), Input(:,2), Target)
hold on
K3 = classifier_trained.W(1,1)
L3 = classifier_trained.W(1,2:end);
% Plot the curve K + [x,y]*L + [x,y]*Q*[x,y]' = 0:
f3 = @(x,y) K3 + L3(1)*x + L3(2)*y
% f3 = @(x,y) 97.5 - 6*x - 5*y
h = ezplot(f3, [-1000. 1000]);
